...
def build_analytics_context():
    ...
    for p in top3:
        lines.append(f"  ✅ views={p.get('views',0)} likes={p.get('likes',0)} — [{p.get('category','?')}] {p.get('text','')[:60]}")
    if flop3:
...
TOPICS = [
    ...
    {"text": "Провокаційна думка про ринок реклами або digital маркетинг.", "category": "opinion"},
...
Пишеш пости в Threads. Твоя мета: збирати охват на особистих постах і залучати клієнтів.
...
Стиль: жива мова, як пише реальна людина — без шаблонів
Довжина: 3–6 речень, максимум 500 символів
Без хештегів, максимум 1–2 емодзі
НЕ згадуй Telegram в постах — тільки у відповідях"""
    if analytics_ctx:
        base += f"\n\n{analytics_ctx}\nАдаптуй стиль на основі аналітики."
    return base

REPLY_SYSTEM = f"""Ти — Міла, AI-асистент Антона Дячі. Відповідаєш на коментарі в Threads від його імені.

Твоя мета: допомогти людині і, якщо є потреба — запросити на онлайн зустріч з Антоном.

Правила:
1. ПЕРШИЙ контакт: задай 1 конкретне запитання про бізнес/потребу
2. Є інтерес → запропонуй зустріч: "Напишіть Антону в ТГ: {TELEGRAM_LINK}"
3. ТЕПЛИЙ КОНТАКТ (3+ коментарі) → особистий тон, пряме запрошення на зустріч
4. Нейтральний коментар → коротка щира подяка
5. Негатив/тролінг → skip
6. Мова: мікс укр/рус, жива
7. 1–3 речення максимум

Відповідай тільки текстом. Якщо не треба — напиши: skip"""

# ─── АНАЛІТИКА ────────────────────────────────────────────────────────────────
def run_analytics(user_id):
    print("📊 Збираю аналітику постів за 7 днів...")
    analytics = load_analytics()
    kb = load_kb()
    ...
    for post in posts:
        ...
        updated += 1
        print(f"  📈 views={insights.get('views',0)} likes={insights.get('likes',0)} [{existing.get('category','?')}]")
    ...
    if all_posts:
        ...
        for p in all_posts:
            ...
            if cat not in by_cat:
                by_cat[cat] = {"views": 0, "count": 0}
            by_cat[cat]["views"] += p.get("views", 0)
            by_cat[cat]["count"] += 1
        cat_avg = {k: v["views"] / v["count"] for k, v in by_cat.items()}
...
def run_post(user_id):
    ...
    for i, topic in enumerate(selected):
        try:
            ...
            print(f"  ✅ {thread_id}")
            analytics[thread_id] = {
                "id": thread_id, "text": text[:200], "category": topic["category"],
        ...
        for reply in replies:
            ...
            try:
                context = (
                    ...
                    f"Стадія: {stage} (0=перший контакт, 1=вже запитав)\n"
                    f"Активність: {warmth} коментарів (3+ = теплий, запрошуй на зустріч)"
                )
...
if __name__ == "__main__":
    ...
    try:
        me = get_user_id()
        user_id = me["id"]
        print(f"👤 Threads: @{me.get('username','?')} (id: {user_id})")
    except Exception as e:
